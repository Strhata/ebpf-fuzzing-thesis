...
// This file has been changed from the original google/buzzer source
// (pkg/strategies/coverage_based.go) for the coverage-race benchmark: added the
// -coverage_log and -max_programs CLI flags and a CSV logger that appends
// (elapsed_ms, programs_submitted, valid_programs, unique_pcs) after each valid
// program. Licensed under the Apache License 2.0.
// ---------------------------------------------------------------------------
...
var (
	coverageLogPath  = flag.String("coverage_log", "/mnt/corpus/buzzer_coverage.csv", "Path to write the coverage race CSV log")
	maxPrograms      = flag.Int("max_programs", 0, "Stop after this many programs submitted (0 = run forever)")
)
...
func NewCoverageBasedStrategy() *CoverageBased {
	...
	logFile, err := os.OpenFile(*coverageLogPath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0644)
	if err != nil {
		fmt.Printf("Warning: could not open coverage log %s: %v\n", *coverageLogPath, err)
		logFile = nil
	} else {
		fmt.Fprintf(logFile, "elapsed_ms,programs_submitted,valid_programs,unique_pcs\n")
	}
...
func (cv *CoverageBased) OnVerifyDone(ffi *units.FFI, verificationResult *fpb.ValidationResult) bool {
	...
	if cv.logFile != nil {
		elapsed := time.Since(cv.startTime).Milliseconds()
		fmt.Fprintf(cv.logFile, "%d,%d,%d,%d\n", elapsed, cv.programCount, cv.validProgramCount, len(cv.coverageHashTable))
	}
...
func (cv *CoverageBased) IsFuzzingDone() bool {
	if *maxPrograms > 0 && cv.programCount >= *maxPrograms {
		if cv.logFile != nil {
			cv.logFile.Close()
		}
		return true
