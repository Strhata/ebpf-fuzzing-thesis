...
func (e *FFI) ValidateEbpfProgram(encodedProgram *fpb.EncodedProgram) (*fpb.ValidationResult, error) {
	...
	// Usa i getter corretti generati da Protobuf
	verifierLog := res.GetVerifierLog()
	isValid := res.GetIsValid()

	// Se il programma è invalido, parsiamo l'errore dal verifier
	if !isValid && verifierLog != "" {
		reLine := regexp.MustCompile(`(?m)^(\d+):\s+\(`)
		matches := reLine.FindAllStringSubmatch(verifierLog, -1)
		if len(matches) > 0 {
			fmt.Sscanf(matches[len(matches)-1][1], "%d", &errorLine)
		}

		lines := strings.Split(strings.TrimSpace(verifierLog), "\n")
		if len(lines) > 0 {
			lastLine := lines[len(lines)-1]
			if !strings.Contains(lastLine, ": (") {
				errorReason = lastLine
			}
		}
	}

	// encodedProgram.GetProgram() ci dà l'array di byte del bytecode eBPF
	sample := MLTrainingSample{
		BytecodeHex: fmt.Sprintf("%x", encodedProgram.GetProgram()),
	...
	pid := os.Getpid()
	fileName := fmt.Sprintf("/mnt/corpus/dataset_%s_%d.jsonl", hostname, pid)

	if jsonData, jsonErr := json.Marshal(sample); jsonErr == nil {
